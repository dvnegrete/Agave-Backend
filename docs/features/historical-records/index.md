...
### 5.2 GET `/unclaimed-deposits`

**Description:** Retrieve deposits without assigned house or conceptos.

**Query Parameters:**
- `startDate`: Optional (ISO date)
- `endDate`: Optional (ISO date)
- `validationStatus`: 'conflict' | 'not-found' | 'all' (default: shows both)
- `houseNumber`: Optional filter by suggested house number
  ...
  "items": [
    {
      ...
      "date": "2023-01-15",
      "time": "14:30:00",
      "concept": "Depósito histórico",
      "validationStatus": "not-found",
      "reason": "Registro histórico importado - Sin casa (será conciliado manualmente)",
      "suggestedHouseNumber": 42,
      "processedAt": "2024-01-08T10:00:00Z"
    }
  ]
}
```

**Service:** `UnclaimedDepositsService`

**Shows:**
- Casa = 0 records (validation_status = 'not-found')
- Casa > 0 records with sum(cta_*) = 0 (validation_status = 'confirmed')
- Conflict records (validation_status = 'conflict')

---

### 5.3 POST `/unclaimed-deposits/{id}/assign-house`

**Description:** Manually assign a house to an unidentified deposit.

**Request:**
- Method: `POST`
- Path Parameter: `id` (transactionBankId)
- Body:
```json
{
  "houseNumber": 42,
  "userId": "user-uuid",
  "adminNotes": "Confirmed with accounting records"
}
```
...
```json
{
  "message": "Depósito asignado exitosamente a casa 42",
  "reconciliation": {
    "transactionBankId": "12345",
    "houseNumber": 42,
    "status": "confirmed",
    "paymentAllocation": {
      "total_distributed": 1542.00,
      "allocations": [
        {
...
**Service:** `UnclaimedDepositsService.assignHouseToDeposit()`

**Actions:**
1. Updates `transactions_status.validation_status` to 'confirmed'
2. Creates `records` entry
3. Creates `house_records` association
4. Creates `manual_validation_approvals` audit record
5. Attempts to allocate payment to concepts via `AllocatePaymentUseCase`
